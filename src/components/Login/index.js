...
const Login = props => {
  ...
  const onSubmitOfForm = async event => {
    ...
    // // Method -II Without using Database fixing user name "raja", password "raja@2021"
    // if (
    //   username === dummyUserCredentials.userName &&
    //   password === dummyUserCredentials.userPassword
    // ) {
    //   const {history} = props
    //   Cookies.set('jwt_token', username, {expires: 30, path: '/'})
    //   history.replace('/')
    // } else if (
    //   username !== dummyUserCredentials.userName &&
    //   password !== dummyUserCredentials.userPassword &&
    ...
    //   }))
    // } else if (username !== dummyUserCredentials.userName && username !== '') {
    //   setState(prevState => ({...prevState, errorMsg: '*Incorrect Username'}))
    // } else if (
    //   password !== dummyUserCredentials.userPassword &&
    //   password !== ''
    // ) {
    //   setState(prevState => ({...prevState, errorMsg: '* Incorrect password'}))
    // } else if (username === '') {
    //   setState(prevState => ({...prevState, errorMsg: '*Provide username'}))
    // } else {
    //   setState(prevState => ({...prevState, errorMsg: '*Provide password'}))
    // }
  }
